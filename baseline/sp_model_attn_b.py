...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.bi_att_2 = BiAttention(config.hidden*2, 1-config.keep_prob,2)
        self.linear_bi_att_2 = nn.Sequential(
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        output_bi_att_2 = self.bi_att_2(output, ques_output, ques_mask)
        output_bi_att_2 = self.linear_bi_att_2(output_bi_att_2)

        output_t = self.rnn_2(output_bi_att_2, context_lens)
...
class BiAttention(nn.Module):
    def __init__(self, input_size, dropout,num =0 ):
        super().__init__()
        self.num = num
        self.dropout = LockedDropout(dropout)
        print(self.num)
        print(dropout," mask size")
